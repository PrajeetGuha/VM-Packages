...
# check if install script is present or not
def check_install_script_present(
    package,
):
    install_script_path = f"packages/{package}/tools/chocolateyinstall.ps1"
    if Path(install_script_path).is_file():
        return True
    else:
        return False
...
def update_github_url(
    ...
    for (
        ...
        latest_version_match = get_latest_version(
            org,
            project,
            version,
        )
...
def update_dynamic_url(
    ...
    if re.fullmatch(
        ...
        matches_url = re.findall(
            r"[\"'](https{0,1}://[^\"']+)[\"']",
            content,
        )
        matches_hash = re.findall(
            r"[\"']([a-fA-F0-9]{64})[\"']",
            content,
        )
        ...
        latest_version = update_nuspec_version(
            package,
            version,
        )
        return latest_version
